...
from oscar_accounts import exceptions as act_exceptions, security
...
class PaymentDetailsView(views.PaymentDetailsView):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_for_allocation = (
            order_total.incl_tax if order_total.is_tax_known else order_total.excl_tax
        )
    ...
    def handle_payment(self, order_number, total, **kwargs):
        ...
        for code, amount in allocations.items():
            source = Source(
                source_type=source_type, amount_debited=amount, reference=code
            )
    ...
    def select_account(self, request):
        ...
        if security.is_blocked(request):
            messages.error(request, "You are currently blocked from using accounts")
        ...
        ctx["allocation_form"] = forms.AllocationForm(
            ...
            else ctx["shipping_charge"].excl_tax,
            ctx["order_total"].incl_tax
            if ctx["order_total"].is_tax_known
            else ctx["order_total"].excl_tax,
    ...
    def add_allocation(self, request):
        ...
        account_form = forms.ValidAccountForm(request.user, self.request.POST)
        if not account_form.is_valid():
            # Only manipulation can get us here
            messages.error(request, _("An error occurred allocating from your account"))
            return http.HttpResponseRedirect(reverse("checkout:payment-details"))
    ...
    def remove_allocation(self, request):
        ...
        if not allocations.contains(code):
            messages.error(request, _("No allocation found with code '%s'") % code)
        else:
    ...
    def set_account_allocations(self, allocations):
        return self.checkout_session._set(
            "accounts", "allocations", Allocations.serialize(allocations)
        )
